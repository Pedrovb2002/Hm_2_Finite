...
def get_physical_groups_map(gmshmodel):
    ...
    names. The map will return the dimension and tag of the physical group if
    indexed by name.
    """
...
def get_elements_and_nodes_in_physical_group(groupname, gmshmodel):
    """
    Returns element tags, node tags (connectivity), element name (gmsh element
    type name), and number of nodes for the element type, given the name of a
    physical group. Inputs are the physical group string name and the gmsh model.
    """
    ...
    for e in entities:
        elementTypes, elementTags, nodeTags = gmshmodel.mesh.getElements(dim, e)
        ...
        allnodetags = concatenate((allnodetags, nodeTags[0]))
...
def get_element_info_from_elementType(elementType):
    ...
    if elementType in info:
        ...
        print(f"elementType={elementType} unavailable. Contributions welcome. "
